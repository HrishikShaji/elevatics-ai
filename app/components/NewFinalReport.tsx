...
export default function NewFinalReport() {
  ...
  async function handleEmail() {
    ...
    if (response.ok) {
      ...
      a.remove();
      window.URL.revokeObjectURL(url);
      sendPdfToBackend(blob, email);
    } else {
      console.error("Failed to share PDF");
    }
  }
  async function sendPdfToBackend(pdfBlob: Blob, email: string) {
    ...
    if (response.ok) {
      console.log("PDF successfully sent to the backend");
    } else {
      console.error("Failed to send PDF to the backend");
    }
  }
        ...
        {isShare ? (
          <div className="fixed z-50 top-0 left-0 h-screen w-full bg-black/70 flex items-center justify-center">
            <div className="relative  p-2 flex flex-col gap-2 bg-white rounded-3xl">
              ...
              <div className="flex flex-col gap-5 p-5 items-center justify-center">
                <ShareEmail email={email} setEmail={setEmail} />
              </div>
            </div>
          </div>
        ) : null}
